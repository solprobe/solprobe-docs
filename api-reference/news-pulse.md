...
The cheapest sentiment read. A lexicon-only pulse of the live Twitter cashtag stream for a Solana token — a bullish/bearish/neutral signal with counts and top terms, no LLM in the loop.
...
- A fast, cheap sentiment sweep — no LLM cost.

For an LLM synthesis grounded against on-chain structure, use [News brief](news-brief.md).
